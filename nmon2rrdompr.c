...
{     
	...
	while ( -1 != (i = getopt(argc, argv, "?f:d:r:xw:h:n:gb" ))) {
		switch (i) {
		case '?':
			hint();
			exit(0);
  ...
  printf("Refreshtime has to be >5 Seconds");
  }}
       ...
       if ( (n = findfirst("AAA,OS,Linux")) != -1){
		linuxone=1; 
		 ...
		 if(strstr(OS,"x86_64"))
          ...
          /*BBBP,PRETTY LINUX OS NAME*/
          if ( (n = findfirst("BBBP,006,/etc/release"))!=-1){
            OSNAME=&line[n][36];
             
		         OS="\"Linux ";
		 char *namepretty=(char *) malloc(1+strlen(OS)+strlen(OSNAME));
                 strcpy(namepretty,OS);
                 strcat(namepretty,OSNAME);
                 OSNAMEPRETTY=namepretty;
                 printf("os line %s\n",namepretty);}
...
int	i;
	...
	FILE * fp;
        ...
        char *infile="rhel7_160731_0606.nmon";
...
printf("where is the problem1,2\n");
	if ( (fp = fopen(infile, "r")) == NULL) {
		perror("failed to open file");
		printf("file: \"%s\"\n", infile);
                printf("err %d \n", errno);
...
printf("where is the problem1,3\n");
	...
	linemax = i;
	lines = i;
printf("where is the problem1,4\n");
	if (debug)
		for (i = 0; i < linemax; i++)
			printf("line %d lastline %s\n", i, line[i-1]);
