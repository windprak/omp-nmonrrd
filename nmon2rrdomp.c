...
struct Reader{
	...
	int longest;
		...
		struct Reader ReadFile();
struct Reader ReadFile(char *filename)
...
int len;
   ...
   if (handler)
   {
	   ...
	   for (i = 0; fgets(string, STRLEN, handler) != NULL; i++) {
    if (i >= lines) {
        lines += 1024;
...
struct Reader file= ReadFile(infile);
	
	line = file.line;
        linemax=file.linemax;
	lines=file.lines;
	s=file.s;
	longest = file.longest;

printf("linemax %d and lines %d\n",linemax,lines);
cfp= NULL;
gfp = NULL;	
